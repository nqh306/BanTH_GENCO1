...
chat_id_admin = "-4056409747"
chat_id = '-1001940344495'

async def source_send_telegram(textMessage, chat_id):
    try:
        token = '6422892988:AAGUP63bMTfcP6NXZJ49-CxKU66kN6R1_-0'
        telegram_notify = telegram.Bot(token=token)
        await asyncio.wait_for(telegram_notify.send_message(chat_id=chat_id, text=textMessage, parse_mode='Markdown'), timeout=10)
    except asyncio.TimeoutError:
        print("Timeout: The message sending operation took too long.")
    except telegram.error.TelegramError as tele_error:
        print(f"Telegram Error: {tele_error}")
    except Exception as ex:
        print(f"An error occurred: {ex}")

async def source_send_file_telegram(file_path, chat_id):
    try:
        with open(file_path, 'rb') as file:
            token = '6422892988:AAGUP63bMTfcP6NXZJ49-CxKU66kN6R1_-0'
            telegram_notify = telegram.Bot(token=token)
            await asyncio.wait_for(telegram_notify.send_document(chat_id=chat_id, document=file), timeout=10)
    except asyncio.TimeoutError:
        print("Timeout: The message sending operation took too long.")
    except telegram.error.TelegramError as tele_error:
        print(f"Telegram Error: {tele_error}")
...
def send_telegram(textMessage):
    asyncio.run(source_send_telegram(textMessage, chat_id))

def send_telegram_admin(textMessage):
    asyncio.run(source_send_telegram(textMessage, chat_id_admin))
    ...
    try:
        if is_dataframe_empty(data_frame):
            ...
            if replace_data == True:
                ...
                #worksheet.insert_rows(data_to_insert, index=num_rows + 1)
                worksheet.append_rows(data_to_insert, value_input_option='RAW')
...
def upload_dataframe_to_google_sheet_with_2_condition(token_file, spreadsheet_id, worksheet_name, data_frame, replace_data=True, column_name_to_delete_1=None, id_values_to_delete_1=None, column_name_to_delete_2=None, id_values_to_delete_2=None):
    data_frame = data_frame.replace([np.inf, -np.inf], np.nan).fillna('')
    try:
        if is_dataframe_empty(data_frame):
            return False, "DataFrame is empty for worksheet: " + str(worksheet_name)
            ...
            if replace_data == True:
                ...
                if len(worksheet.get_all_values()) > 1:
                    ...
                    data = data[1:]
                    # Find the index of the ID column
                    id_column_index_1 = header.index(column_name_to_delete_1)
                    id_column_index_2 = header.index(column_name_to_delete_2)
                    # Create a new data list excluding rows with specified ID values
                    new_data = [row for row in data if row[id_column_index_1] not in id_values_to_delete_1 or row[id_column_index_2] not in id_values_to_delete_2]
                    # Clear the existing data in the worksheet
                    worksheet.clear()
                    # Insert the new data
                    if new_data:
...
def get_listValue_from_col_in_GoogleSheet(token, spreadsheet_id, worksheet_name, column_name):
    try:
        ...
        spreadsheet = client.open_by_key(spreadsheet_id)
        worksheet = spreadsheet.worksheet(worksheet_name)
        # Get all header values (first row) to map column names to column numbers
        header_row = worksheet.row_values(1)
        # Find the column number corresponding to the provided column name
        try:
            column_number = header_row.index(column_name) + 1
        except ValueError:
            raise ValueError(f"Column name '{column_name}' not found in the worksheet")
...
def get_listValue_from_col_in_GoogleSheet_with_condition(token, spreadsheet_id, worksheet_name, column_name_need, column_name_condition, text_match_condition):
    try:
        # Define the scope of access (in this case, just Google Sheets)
        scope = ["https://www.googleapis.com/auth/spreadsheets.readonly"]
        # Initialize the credentials and client
        ...
        # Get all header values (first row) to map column names to column numbers
        header_row = worksheet.row_values(1)
        # Find the column numbers corresponding to the provided column names
        try:
            column_number_need = header_row.index(column_name_need) + 1
        except ValueError:
            raise ValueError(f"Column name '{column_name_need}' not found in the worksheet")
        
        try:
            column_number_condition = header_row.index(column_name_condition) + 1
        except ValueError:
            raise ValueError(f"Column name '{column_name_condition}' not found in the worksheet")
        ...
        for row in worksheet.get_all_values():
            if row[column_number_condition - 1] == text_match_condition:  # Adjust for 0-based indexing
                matching_values_set.add(row[column_number_need - 1])  # Adjust for 0-based indexing
...
def get_values_from_2_columns_with_condition(token, spreadsheet_id, worksheet_name, column_name1, column_name2, column_name_condition, text_match_condition):
    try:
        # Define the scope of access (in this case, just Google Sheets)
        scope = ["https://www.googleapis.com/auth/spreadsheets.readonly"]
        # Initialize the credentials and client
        ...
        # Find the column numbers corresponding to the provided column names
        try:
            column_number1 = header_row.index(column_name1) + 1
        except ValueError:
            raise ValueError(f"Column name '{column_name1}' not found in the worksheet")

        try:
            column_number2 = header_row.index(column_name2) + 1
        except ValueError:
            raise ValueError(f"Column name '{column_name2}' not found in the worksheet")
        ...
        for row in worksheet.get_all_values():
            if row[column_number_condition - 1] == text_match_condition:  # Adjust for 0-based indexing
                matching_values.append((row[column_number1 - 1], row[column_number2 - 1]))  # Adjust for 0-based indexing
...
def fetch_data_from_api(url, payload, headers):
    ...
    while retry_count < max_retries:
        try:
            ...
            if retry_count > 0:
                print("fetch data successfull in " + str(retry_count + 1) + " times")
            break
...
def process_get_chitiet_KHLCNT_GOITHAU(id_KHLCNT):
    ...
    if data_api_KHLCNT_ct:
        ...
        for j in range(len(data_api_KHLCNT_ct['lsBidpPlanDetailDTO'])):
            ...
            if NGUON_VON is not None:
                nguon_von_upper = NGUON_VON.upper()
                nguon_von_mapping = {
                ...
                for key, values in nguon_von_mapping.items():
                    if any(value in nguon_von_upper for value in values):
                        PHANLOAI_NGUONVON = key
                        break
            
            GIA_GOITHAU = data_api_KHLCNT_ct['lsBidpPlanDetailDTO'][j]["bidPrice"]
            TEN_DONVI = MAPPING_TENDONVI[str(data_api_KHLCNT_ct['bidpPlanDetailToProjectList'][0]['createdBy']).split("-")[0]]
            THAMQUYEN_PHEDUYET = ""
...
def check_status_TBMT_if_changed_get_info_GOITHAU(status_search):
    ...
    khlcnt_data = get_values_from_2_columns_with_condition(token_QLDT_GoogleSheet, spreadsheetID_QLDT, "2.1.GOI_THAU", "planID", "MA_TBMT", "STATUS_TBMT", status_search)
...
def main():
    ...
    list_id_khlcnt_googleSheet = get_listValue_from_col_in_GoogleSheet(token_QLDT_GoogleSheet, spreadsheetID_QLDT, "2.1.KHLCNT", "planID")
    list_ma_KHLCNT_googleSheet = get_listValue_from_col_in_GoogleSheet(token_QLDT_GoogleSheet, spreadsheetID_QLDT, "2.1.KHLCNT", "planNo")
    ...
    if list_khlcnt:
        for row_list_khlcnt in range(len(list_khlcnt)):
            ...
            if list_khlcnt[row_list_khlcnt]["id"] not in list_id_khlcnt_googleSheet:
                ...
                if list_khlcnt[row_list_khlcnt]["planNo"] not in list_ma_KHLCNT_googleSheet:
                    ...
                    if not df_new_GOITHAU.empty:
                        df_new_GOITHAU['ACTION'] = ""
                        df_GOITHAU = pd.concat([df_GOITHAU, df_new_GOITHAU], ignore_index=True)

    #LẤY THÔNG TIN GÓI THẦU MỚI ĐĂNG TẢI TBMT:
    print("II. KIỂM TRA GÓI THẦU MỚI ĐĂNG TẢI TBMT")
    list_id_KHLCNT_not_yet_TBMT = get_listValue_from_col_in_GoogleSheet_with_condition(token_QLDT_GoogleSheet,spreadsheetID_QLDT,"2.1.GOI_THAU","planID","STATUS_TBMT","01. Chưa có TBMT")
    ...
    list_maTBMT = get_listValue_from_col_in_GoogleSheet(token=token_QLDT_GoogleSheet,spreadsheet_id=spreadsheetID_QLDT,worksheet_name="2.1.GOI_THAU_CT",column_name="MA_TBMT")
    ...
    #LẤY THÔNG TIN CẬP NHẬT GIÁ DỰ TOÁN
    print("III.5. KIỂM TRA CẬP NHẬT DỰ TOÁN + GIA HẠN + LÀM RÕ + KIẾN NGHỊ")
    list_IDTBMT = get_values_from_2_columns_with_condition(token=token_QLDT_GoogleSheet, spreadsheet_id=spreadsheetID_QLDT, worksheet_name="2.1.GOI_THAU_CT",
                                                           column_name1="ID_GOITHAU",column_name2="DUTOAN_GOITHAU",column_name_condition="STATUS_TBMT",text_match_condition="02. Chưa đóng thầu"
                                                           ) + get_values_from_2_columns_with_condition(token=token_QLDT_GoogleSheet, spreadsheet_id=spreadsheetID_QLDT, worksheet_name="2.1.GOI_THAU_CT",
                                                           column_name1="ID_GOITHAU",column_name2="DUTOAN_GOITHAU",column_name_condition="STATUS_TBMT",text_match_condition="03. Đang xét thầu")
    List_ID_GIAHAN = get_listValue_from_col_in_GoogleSheet(token=token_QLDT_GoogleSheet,spreadsheet_id=spreadsheetID_QLDT,worksheet_name="2.1.GIA_HAN", column_name="ID_GIAHAN")
    List_ID_LAMRO = get_listValue_from_col_in_GoogleSheet(token=token_QLDT_GoogleSheet,spreadsheet_id=spreadsheetID_QLDT,worksheet_name="2.1.LAM_RO",column_name="id")
    List_ID_KIENNGHI = get_listValue_from_col_in_GoogleSheet(token=token_QLDT_GoogleSheet,spreadsheet_id=spreadsheetID_QLDT,worksheet_name="2.1.KIEN_NGHI",column_name="reqFileId")
    
    for row in list_IDTBMT:
        data_get_by_id = fetch_data_from_api(url=f"{API_BASE_URL}/expose/lcnt/bid-po-bido-notify-contractor-view/get-by-id",
                                                        payload=json.dumps({"id": str(row[0])}),
                                                        headers=API_HEADER)
        #CHECK CẬP NHẬT GIÁ DỰ TOÁN
        if data_get_by_id['bidoNotifyContractorM'] != None:
            if 'bidEstimatePrice' in data_get_by_id['bidoNotifyContractorM']:
                if data_get_by_id['bidoNotifyContractorM']['bidEstimatePrice'] != row[1]:
                    print(str(data_get_by_id['bidoNotifyContractorM']['notifyNo']) + " cập nhật giá dự toán")
                    if df_telegram.empty:
                        telegram_count = 0
                    else:
                        telegram_count = len(df_telegram)
                    data_telegram = {
                                    'No': [telegram_count + 1],
                                    'TextMessage': ["🌟 CẬP NHẬT GIÁ DỰ TOÁN 🌟" + '\n' + 
                                                    "Đơn vị: " + MAPPING_TENDONVI[str(data_get_by_id['bidNoContractorResponse']['bidNotification']['procuringEntityCode'])] + '\n' +
                                                    "Mã TBMT: " + str(data_get_by_id['bidoNotifyContractorM']['notifyNo']) + '\n' +
                                                    "Tên gói thầu: " + str(data_get_by_id['bidNoContractorResponse']['bidNotification']['bidName']) + '\n' +
                                                    "Giá dự toán mới: " + f"{data_get_by_id['bidoNotifyContractorM']['bidEstimatePrice']:,}" + ' VNĐ'
                                                    ],
                                    'Date_get_data': [datetime.now().strftime("%Y-%m-%d %H:%M:%S")]}
                    df_telegram = pd.concat([df_telegram, pd.DataFrame(data_telegram)], ignore_index=True)

                    df_new_goithau = get_info_GoiThau_by_MaTBMT_from_id_KHLCNT(data_get_by_id['bidpPlanDetail']['planId'], data_get_by_id['bidoNotifyContractorM']['notifyNo'])
                    if not df_new_goithau.empty:
                        df_new_GOITHAU['ACTION'] = "UPDATE GIA DU TOAN"
                        # Concatenate the new data to the existing DataFrame
                        df_goithau = pd.concat([df_goithau, df_new_goithau], ignore_index=True)
        #CHECK GIA HẠN GÓI THẦU
        if data_get_by_id['bidNoContractorResponse'] != None:
                    if data_get_by_id['bidNoContractorResponse']['bidNotification'] != None:
                        if 'delayDTOList' in data_get_by_id['bidNoContractorResponse']['bidNotification']:
                            if data_get_by_id['bidNoContractorResponse']['bidNotification']['delayDTOList'] != None:
                                for row_json in range(0,len(data_get_by_id['bidNoContractorResponse']['bidNotification']['delayDTOList'])):
                                    if data_get_by_id['bidNoContractorResponse']['bidNotification']['delayDTOList'][row_json]["id"] not in List_ID_GIAHAN:
                                        if df_telegram.empty:
                                            telegram_count = 0
                                        else:
                                            telegram_count = len(df_telegram)
                                        data_telegram = {
                                                        'No': [telegram_count + 1],
                                                        'TextMessage': ["🌟 GIA HẠN GÓI THẦU 🌟" + '\n' + 
                                                                        "Đơn vị: " + MAPPING_TENDONVI[str(data_get_by_id['bidNoContractorResponse']['bidNotification']['procuringEntityCode'])] + '\n' +
                                                                        "Mã TBMT: " + str(data_get_by_id['bidoNotifyContractorM']['notifyNo']) + '\n' +
                                                                        "Tên gói thầu: " + str(data_get_by_id['bidNoContractorResponse']['bidNotification']['bidName']) + '\n' +
                                                                        "Lần gia hạn: " + "Lần " + str(row_json + 1) + '\n' +
                                                                        "Lý do: " + str(data_get_by_id['bidNoContractorResponse']['bidNotification']['delayDTOList'][row_json]["reason"]) + '\n' +
                                                                        "Ngày gia hạn: " + str(data_get_by_id['bidNoContractorResponse']['bidNotification']['delayDTOList'][row_json]["createdDate"]) + '\n' +
                                                                        "Thời điểm mở thầu cũ: " + str(data_get_by_id['bidNoContractorResponse']['bidNotification']['delayDTOList'][row_json]["bidOpenDate"]) + '\n' +
                                                                        "Thời điểm đóng thầu cũ: " + str(data_get_by_id['bidNoContractorResponse']['bidNotification']['delayDTOList'][row_json]["bidCloseDate"]) + '\n' +
                                                                        "Thời điểm mở thầu mới: " + str(data_get_by_id['bidNoContractorResponse']['bidNotification']['delayDTOList'][row_json]["bidOpenDelayDate"]) + '\n' +
                                                                        "Thời điểm đóng thầu mới: " + str(data_get_by_id['bidNoContractorResponse']['bidNotification']['delayDTOList'][row_json]["bidCloseDelayDate"])
                                                                        ],
                                                        'Date_get_data': [datetime.now().strftime("%Y-%m-%d %H:%M:%S")]}
                                        df_telegram = pd.concat([df_telegram, pd.DataFrame(data_telegram)], ignore_index=True)
                                        ...
                                        df_new_goithau = get_info_GoiThau_by_MaTBMT_from_id_KHLCNT(data_get_by_id['bidpPlanDetail']['planId'], data_get_by_id['bidoNotifyContractorM']['notifyNo'])
                                        if not df_new_goithau.empty:
                                            df_new_GOITHAU['ACTION'] = "GIA HAN"
                                            # Concatenate the new data to the existing DataFrame
                                            df_goithau = pd.concat([df_goithau, df_new_goithau], ignore_index=True)
        
        #CHECK GOI THẦU PHÁT SINH LÀM RÕ
        if data_get_by_id["biduClarifyReqInvAndContentViewList"] != []:
            if data_get_by_id["biduClarifyReqInvAndContentViewList"] != None:
                for row_json in range(0,len(data_get_by_id["biduClarifyReqInvAndContentViewList"])):
                    if data_get_by_id["biduClarifyReqInvAndContentViewList"][row_json]["id"] not in List_ID_LAMRO:
                        if df_telegram.empty:
                            telegram_count = 0
                        else:
                            telegram_count = len(df_telegram)
                        data_telegram = {
                                        ...
                                        'TextMessage': ["🌟 LÀM RÕ 🌟" + '\n' + 
                                                        ...
                                                        "Mã TBMT: " + str(data_get_by_id['bidoNotifyContractorM']['notifyNo']) + '\n' +
                                                        "Tên gói thầu: " + str(data_get_by_id['bidNoContractorResponse']['bidNotification']['bidName']) + '\n' +
                                                        "Ngày yêu cầu làm rõ: " + str(data_get_by_id["biduPetitionContractorDTOList"][row_json]["reqDate"]) + '\n' +
                                                        "Tên yêu cầu làm rõ: " + str(data_get_by_id["biduPetitionContractorDTOList"][row_json]["reqName"]) + '\n' +
                                                        "[Vui lòng vào app Ban QLDT để xem chi tiết]"
                                                        ],
                                        'Date_get_data': [datetime.now().strftime("%Y-%m-%d %H:%M:%S")]}
                        df_telegram = pd.concat([df_telegram, pd.DataFrame(data_telegram)], ignore_index=True)
                        df_new_goithau = get_info_GoiThau_by_MaTBMT_from_id_KHLCNT(data_get_by_id['bidpPlanDetail']['planId'], data_get_by_id['bidoNotifyContractorM']['notifyNo'])
                        if not df_new_goithau.empty:
                            # Concatenate the new data to the existing DataFrame
                            df_new_GOITHAU['ACTION'] = "LAM RO"
                            df_goithau = pd.concat([df_goithau, df_new_goithau], ignore_index=True)
        #CHECK GOI THẦU PHÁT SINH KIẾN NGHỊ
        if data_get_by_id["biduPetitionContractorDTOList"] != []:
            if data_get_by_id["biduPetitionContractorDTOList"] != None:
                for row_json in range(0,len(data_get_by_id["biduPetitionContractorDTOList"])):
                    if data_get_by_id["biduPetitionContractorDTOList"][row_json]["id"] not in List_ID_KIENNGHI:
                        if df_telegram.empty:
                            telegram_count = 0
                        else:
                            telegram_count = len(df_telegram)
                        data_telegram = {
                                        'No': [telegram_count + 1],
                                        'TextMessage': ["🌟 KIẾN NGHỊ 🌟" + '\n' + 
                                                        "Đơn vị: " + MAPPING_TENDONVI[str(data_get_by_id['bidNoContractorResponse']['bidNotification']['procuringEntityCode'])] + '\n' +
                                                        "Mã TBMT: " + str(data_get_by_id['bidoNotifyContractorM']['notifyNo']) + '\n' +
                                                        "Tên gói thầu: " + str(data_get_by_id['bidNoContractorResponse']['bidNotification']['bidName']) + '\n' +
                                                        "Ngày kiến nghị: " + str(data_get_by_id["biduPetitionContractorDTOList"][row_json]["reqDate"]) + '\n' +
                                                        "Tên kiến nghị: " + str(data_get_by_id["biduPetitionContractorDTOList"][row_json]["reqName"]) + '\n' +
                                                        "Đơn vị kiến nghị: " + str(data_get_by_id["biduPetitionContractorDTOList"][row_json]["contractorName"]) + '\n' +
                                                        "[Vui lòng vào app Ban QLDT để xem chi tiết]"
                                                        ],
                                        'Date_get_data': [datetime.now().strftime("%Y-%m-%d %H:%M:%S")]}
                        df_telegram = pd.concat([df_telegram, pd.DataFrame(data_telegram)], ignore_index=True)
                        df_new_goithau = get_info_GoiThau_by_MaTBMT_from_id_KHLCNT(data_get_by_id['bidpPlanDetail']['planId'], data_get_by_id['bidoNotifyContractorM']['notifyNo'])
                        if not df_new_goithau.empty:
                            df_new_GOITHAU['ACTION'] = "KIEN NGHI"
                            # Concatenate the new data to the existing DataFrame
                            df_goithau = pd.concat([df_goithau, df_new_goithau], ignore_index=True)
        ...
        for i in range(0, df_GOITHAU['ID_GOITHAU'].count()):
            ...
            if str(df_GOITHAU['MA_TBMT'][i]) != '':
                ...
                if Ma_TBMT != "IB2300151471": #KHÔNG HIỂU SAO GÓI THẦU NÀY LẠI CÓ CẤU TRÚC KHÁC VỚI TOÀN BỘ CÁC GÓI
                    ...
                    if df_GOITHAU['ACTION'][i] == "TBMT_MOI":
                        ...
                        data_telegram = {
                                        ...
                                        'TextMessage': ["🆕 TBMT MỚI 🌟" + '\n' + 
                                                        ...
                                                        "Thời điểm đóng thầu: " + str(data_get_by_id['bidNoContractorResponse']['bidNotification']['bidCloseDate']) + '\n' +
                        ...
                        for row_mothau in range(len(df_DXKT_new)):
                            if df_DXKT_new["TEN_NHATHAU"][row_mothau] not in THONGTIN_NHATHAU:
                                row_count = row_count + 1
                                KETQUA_DXKT = "Đạt"
                                for row_mothau_lan2 in range(len(df_DXKT_new)):
                                    if df_DXKT_new["TEN_NHATHAU"][row_mothau] == df_DXKT_new["TEN_NHATHAU"][row_mothau_lan2]:
                                        if df_DXKT_new["KETQUA_DANHGIA"][row_mothau_lan2] == "Không đạt":
                                            KETQUA_DXKT = "Không đạt tại bước: " + str(df_DXKT_new["LOAI_DANHGIA"][row_mothau_lan2])
                                if df_DXKT_new["TECH_SCORE"][row_mothau] == None:
                                    THONGTIN_NHATHAU = THONGTIN_NHATHAU + '\n' + str(row_count) + ". " + str(df_DXKT_new["TEN_NHATHAU"][row_mothau]) + ": " + str(KETQUA_DXKT)
                                else:
                                    THONGTIN_NHATHAU = THONGTIN_NHATHAU + '\n' + str(row_count) + ". " + str(df_DXKT_new["TEN_NHATHAU"][row_mothau]) + ": " + str(KETQUA_DXKT) + " (Tech score: " + str(df_DXKT_new["TECH_SCORE"][row_mothau]) + ")"
                        if df_telegram.empty:
                            telegram_count = 0
                        else:
                            telegram_count = len(df_telegram)
                        ...
                        if df_GOITHAU["STATUS_BID"][i] == "IS_PUBLISH":
                            ...
                            data_telegram = {
                                            ...
                                            'TextMessage': ["🌟 CÓ NHÀ THẦU TRÚNG THẦU 🌟" + '\n' + 
                                                            ...
                                                            "Đơn vị trúng thầu: " + THONGTIN_NHATHAU
                                                            ],
                                            'Date_get_data': [datetime.now().strftime("%Y-%m-%d %H:%M:%S")]}
                            df_telegram = pd.concat([df_telegram, pd.DataFrame(data_telegram)], ignore_index=True)
                        else:
                    ...
                    NAM_BAOCAO = max([df_GOITHAU["TGIAN_BATDAU_TOCHUC_LCNT_CHITIET"][i], 
                                        datetime.strptime(str(data_get_by_id['bidNoContractorResponse']['bidNotification']['publicDate']).split(".")[0], "%Y-%m-%dT%H:%M:%S").strftime("%d/%m/%Y"),
                                        datetime.strptime(str(data_get_by_id['bidInvContractorOfflineDTO']['decisionDate']).split(".")[0], "%Y-%m-%dT%H:%M:%S").strftime("%d/%m/%Y"),
                                        HSDXKT_NGAY_QD,
                                        KQLCNT_NGAY_QD,
                                        NGAY_KY_HOPDONG
                                        ])
                    NAM_BAOCAO = int(str(NAM_BAOCAO).split("/")[2])
                    df_GOITHAU.loc[df_GOITHAU['ID_GOITHAU'] == df_GOITHAU["ID_GOITHAU"][i], 'NAM_BAOCAO'] = NAM_BAOCAO
                    ...
                    data_newRow = {'MaDinhDanh': [df_GOITHAU['MaDinhDanh'][i]],
                        ...
                        'PHANLOAI_NGUONVON': [df_GOITHAU["PHANLOAI_NGUONVON"][i]],
                        ...
                        'THAMQUYEN_PHEDUYET': [df_GOITHAU["THAMQUYEN_PHEDUYET"][i]],
    ...
    if not df_ct_khlcnt.empty:
        success, message = upload_dataframe_to_google_sheet("token-qldt.json","1lfkxn5sPh1lqTmyvUe3jSyXNXu2ZG0CSiMEuyId6TaE","2.1.KHLCNT",df_ct_khlcnt,False,"planID",df_ct_khlcnt["planID"].tolist())
        if success == True:
            print(message)
        else:
    ...
    if not df_GOITHAU.empty:
        success, message = upload_dataframe_to_google_sheet_with_2_condition("token-qldt.json","1lfkxn5sPh1lqTmyvUe3jSyXNXu2ZG0CSiMEuyId6TaE","2.1.GOI_THAU",df_GOITHAU,False,"ID_GOITHAU",df_GOITHAU["ID_GOITHAU"].tolist(),"MA_TBMT",df_GOITHAU["MA_TBMT"].tolist())
        if success == True:
            print(message)
        else:
            print(f"Upload failed. Error message: {message}")
    
    if not df_GOITHAU_ct.empty:
        success, message = upload_dataframe_to_google_sheet("token-qldt.json","1lfkxn5sPh1lqTmyvUe3jSyXNXu2ZG0CSiMEuyId6TaE","2.1.GOI_THAU_CT",df_GOITHAU_ct,False,"MA_TBMT",df_GOITHAU_ct["MA_TBMT"].tolist())
        if success == True:
            print(message)
        else:
            print(f"Upload failed. Error message: {message}")

    if not df_GIAHAN.empty:
        success, message = upload_dataframe_to_google_sheet("token-qldt.json","1lfkxn5sPh1lqTmyvUe3jSyXNXu2ZG0CSiMEuyId6TaE","2.1.GIA_HAN",df_GIAHAN,False,"MA_TBMT",df_GIAHAN["MA_TBMT"].tolist())
        if success == True:
            print(message)
        else:
            print(f"Upload failed. Error message: {message}")

    if not df_lamro.empty:
        success, message = upload_dataframe_to_google_sheet("token-qldt.json","1lfkxn5sPh1lqTmyvUe3jSyXNXu2ZG0CSiMEuyId6TaE","2.1.LAM_RO",df_lamro,False,"Ma_TBMT",df_lamro["Ma_TBMT"].tolist())
        if success == True:
            print(message)
        else:
            print(f"Upload failed. Error message: {message}")

    if not df_kiennghi.empty:
        success, message = upload_dataframe_to_google_sheet("token-qldt.json","1lfkxn5sPh1lqTmyvUe3jSyXNXu2ZG0CSiMEuyId6TaE","2.1.KIEN_NGHI",df_kiennghi,False,"MA_TBMT",df_kiennghi["MA_TBMT"].tolist())
        if success == True:
            print(message)
        else:
            print(f"Upload failed. Error message: {message}")
    
    if not df_mothau.empty:
        success, message = upload_dataframe_to_google_sheet("token-qldt.json","1lfkxn5sPh1lqTmyvUe3jSyXNXu2ZG0CSiMEuyId6TaE","2.1.BIENBAN_MOTHAU",df_mothau,False,"Ma_TBMT",df_mothau["Ma_TBMT"].tolist())
        if success == True:
            print(message)
        else:
            print(f"Upload failed. Error message: {message}")

    if not df_DXKT.empty:
        success, message = upload_dataframe_to_google_sheet("token-qldt.json","1lfkxn5sPh1lqTmyvUe3jSyXNXu2ZG0CSiMEuyId6TaE","2.1.HSDXKT",df_DXKT,False,"MA_TBMT",df_DXKT["MA_TBMT"].tolist())
        if success == True:
            print(message)
        else:
            print(f"Upload failed. Error message: {message}")

    if not df_DXTC.empty:
        success, message = upload_dataframe_to_google_sheet("token-qldt.json","1lfkxn5sPh1lqTmyvUe3jSyXNXu2ZG0CSiMEuyId6TaE","2.1.HSDXTC",df_DXTC,False,"Ma_TBMT",df_DXTC["Ma_TBMT"].tolist())
        if success == True:
            print(message)
        else:
            print(f"Upload failed. Error message: {message}")

    if not df_KQLCNT.empty:
        success, message = upload_dataframe_to_google_sheet("token-qldt.json","1lfkxn5sPh1lqTmyvUe3jSyXNXu2ZG0CSiMEuyId6TaE","2.1.KQLCNT",df_KQLCNT,False,"Ma_TBMT",df_KQLCNT["Ma_TBMT"].tolist())
        if success == True:
            print(message)
        else:
            print(f"Upload failed. Error message: {message}")
